...
// Get the QC result
$qcResult = getIndividualInstrumentQCResult($qc);

// We need to get the information of this Assay Module
$instrument = getInstrumentSnapshot($qcResult['instrument']);
...
<section class="flex items-center justify-between gap-4">
    <div class="page-title-area h-full flex items-center gap-4">
        ...
        <h1 class="mb-0">Edit QC Result #<?= $qcResult['id'];?></h1>
    </div>
</section>
...
<section class="notice">
    ...
    <p>Edit the previously logged QC result if there was a mistake..</p>
</section>

<!-- Form for handling QC test submission -->
<form action="/process" method="POST">
    <input type="hidden" name="action" value="edit-instrument-qc">
    <input type="hidden" name="qc-result" value="<?= $qcResult['id'];?>">
    ...
    <div class="form-fields">
        <div class="field">
            ...
            <div class="input-wrapper">
                ...
                <input id="dateTimePicker" name="datetime" class="date-time-picker-no-future" type="text" placeholder="<?= gmdate("Y-m-d");?>" value="<?= date("Y-m-d H:i", ($qcResult['timestamp'] ?? time()));?>" readonly/>
            </div>
        </div>
    </div>
    ...
    <div class="form-fields">
        ...
        <div class="field">
            ...
            <div class="input-wrapper disabled <?= ($hasErrors && $form->getError('qcType')) ? "error" : "";?>">
                <input disabled spellcheck="false" type="text" name="qc-type" value="<?= $qcTestTypes[$qcResult['type']]['name'];?>" class="cursor-not-allowed">
            </div>
        </div>
        ...
        <div class="field">
            ...
            <div class="input-wrapper select-wrapper">
                <select name="result" id="result">
                    <option value="1" <?= $qcResult['result'] == 1 ? "Selected" : "";?>>Pass</option>
                    <option value="0" <?= $qcResult['result'] == 0 ? "Selected" : "";?>>Fail</option>
                </select>
            </div>
        </div>
    </div>
    <div class="form-fields">
        <div class="field">
            ...
            <div class="input-wrapper">
                <textarea class="textarea" name="notes" id="notes"><?= $qcResult['notes'];?></textarea>
            </div>
        </div>
    </div>

    <button type="submit" class="btn smaller-btn trigger-loading">Edit QC Test</button>
